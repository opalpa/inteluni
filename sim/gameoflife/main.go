...
func main() {
  ...
  for comp := complexStart; comp <= complexEnd; comp += complexStep {
    for fs := foresightMin; fs <= foresightMax; fs += foresightStep {
      runSimulation(runID, comp, fs, *seedFlag,)
      runID++
    }
  }
}
